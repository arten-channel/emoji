...
class EmojiKeyboard {
    constructor() {
        this.emojis = new Map(Array.from(categories, v => [v[0], []]));
    ...
    click_on_category(kb, event, stay) {
        ...
        if (!stay) { 
            ...
            if (!targets[0]) return;
            targets[0].parentNode.scrollIntoView(true, { behavior: "instant" }); 
            targets[0].parentNode.parentNode.scrollTop++; 
        }
    }
            ...
            if (contentType && (contentType.includes("application/json") || contentType.includes("text/plain"))) {
                ...
                json.emojis.forEach(emoji => {
                    ...
                    this.emojis.get(categ).push({
                        emoji: emoji.emoji,
                        url: "images/72x72/" + emoji.unicode.replace(/ /g, '-').toLowerCase() + ".png",
